...
function optimizeSvg(writeStream, writeMixinStream, cb, filePath, svgString){
    svgo.optimize(svgString).then(function(result) {
		...
		sassvgIt(writeStream, writeMixinStream, cb, filePath, svgString);
    });
}

let originalSVG = {
	scssFunc : (fileName, svgString) => {
		let svgCode = encodeSVG(svgString);
		return `@mixin sassvg-origin-${fileName} { background-image: url('${DATA_PREFIX}${svgCode}'); background-repeat: no-repeat; }\n`;
	}
}

function sassvgIt(writeStream, writeMixinStream, cb, filePath, svgString){
	if ( writeMixinStream ) {
		writeMixinStream.write(
			originalSVG.scssFunc(fileNameFromPath(filePath), svgString)
		);
	}
	writeStream.write(
		assembleDataString(
			...
			decodeVariables(
				encodeSVG(
					addVariables(filePath, svgString)
				)
			)
		)
	);
...
var gulpSassvg = function(optionsGiven){
	...
	options.outputMixins = optionsGiven.outputMixins || false;
  ...
  if( options.outputFolder.indexOf("/", options.outputFolder.length - 1) === -1 ){
    options.outputFolder = options.outputFolder + "/";
		...
		options.outputMixinFile = options.outputFolder + "_sassvg-mixins.scss";
	...
	let writeMixinStream;
	if ( options.outputMixins === true ) {
		writeMixinStream = fs.createWriteStream(options.outputMixinFile)
	} else {
		writeMixinStream = false;
	}
    ...
    function listStream(file, enc, cb){
		...
		if(options.optimizeSvg){
			optimizeSvg(writeStream, writeMixinStream, cb, file.path, String(file.contents))
		}else{
			sassvgIt(writeStream, writeMixinStream, cb, file.path, String(file.contents));
		}
